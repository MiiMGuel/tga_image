...
int main(int argc, char* argv[]) {
    ...
    tga_color_set(buffer, image->header.width, image->header.height, image->bytespp, 0, 0, white);
    ...
    if (image == NULL) printf("[error] failed reading file!\n"); 
    tga_color_set(buffer, image->header.width, image->header.height, image->bytespp, 0, 0, black);
    tga_image_setc(image, buffer);
